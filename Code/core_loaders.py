...
_ASU_MAX_CHANNELS: int       = 64
_ASU_EOG_CHANNELS: List[int] = [0, 9, 32, 63]
_ASU_MAT_KEY:      str       = "eeg_data_wrt_task_rep_no_eog_256Hz_last_beep"

_ASU_DATASETS: Dict[str, dict] = {
...
def _mat_to_csv(mat_path: str, csv_dir: str, td: int, sfreq: int = _K1_SFREQ_DS) -> None:
    """Convert an ASU ``.mat`` file to flat per-split CSV files.

    Reads the per-class cell array from the ``.mat`` file, applies channel
    capping to ``_ASU_MAX_CHANNELS`` and EOG channel removal, then writes
    three CSV files mirroring the layout expected by ``load_asu``:

    - ``Data/<subject>_Act.csv``    — flat thinking-state trial matrix
    - ``Targets/<subject>_EVE_n.csv`` — integer class labels
    - ``Rest/<subject>_Rst.csv``    — flat resting-state trial matrix
                                      (written only if a resting-state key
                                      exists in the ``.mat`` file)

    Each row in the data CSVs is one flattened trial of shape
    ``(n_channels × samples_per_trial,)``, matching the flat layout that
    ``load_asu`` reshapes back into ``(n_trials, n_channels, samples)``.

    Args:
        mat_path: Path to the raw ``.mat`` file for one subject.
        csv_dir:  Root output directory; ``Data/``, ``Targets/``, and
                  ``Rest/`` subdirectories are created automatically.
        td:       Trial duration in seconds (used to validate sample count).
        sfreq:    Sampling frequency in Hz (default 256).

    Raises:
        KeyError: If the expected thinking-state mat key is absent.
    """
    import scipy.io as sio

    mat       = sio.loadmat(mat_path, squeeze_me=True, struct_as_record=False)
    data_cell = mat[_ASU_MAT_KEY]

    # Infer n_classes from the cell array length.
    n_classes = len(data_cell)
    ...
    for class_idx in range(n_classes):
        ...
        for trial in trials:
            # Step 1 — cap to MAX_CHANNELS.
            if trial.shape[0] > _ASU_MAX_CHANNELS:
                trial = trial[:_ASU_MAX_CHANNELS, :]
            # Step 2 — drop EOG channels.
            valid_ch = [i for i in range(trial.shape[0])
                        if i not in _ASU_EOG_CHANNELS]
            X_list.append(trial[valid_ch, :].flatten())
            y_list.append(class_idx)

    data_arr    = np.stack(X_list, axis=0)
    targets_arr = np.array(y_list, dtype=int)

    data_dir    = os.path.join(csv_dir, "Data")
    tgt_dir     = os.path.join(csv_dir, "Targets")
    os.makedirs(data_dir, exist_ok=True)
    os.makedirs(tgt_dir,  exist_ok=True)

    subject = os.path.splitext(os.path.basename(mat_path))[0]
    np.savetxt(os.path.join(data_dir, f"{subject}_Act.csv"),   data_arr,    delimiter=",")
    np.savetxt(os.path.join(tgt_dir,  f"{subject}_EVE_n.csv"), targets_arr, delimiter=",", fmt="%d")

    # Resting-state — write only if the key exists.
    rest_key = _ASU_MAT_KEY.replace("task", "rest")
    if rest_key in mat:
        rest_cell = mat[rest_key]
        R_list: List[np.ndarray] = []
        for class_idx in range(n_classes):
            class_trials = rest_cell[class_idx]
            trials = [class_trials] if class_trials.ndim == 2 else list(class_trials)
            for trial in trials:
                if trial.shape[0] > _ASU_MAX_CHANNELS:
                    trial = trial[:_ASU_MAX_CHANNELS, :]
                valid_ch = [i for i in range(trial.shape[0])
                            if i not in _ASU_EOG_CHANNELS]
                R_list.append(trial[valid_ch, :].flatten())
        rest_dir = os.path.join(csv_dir, "Rest")
        os.makedirs(rest_dir, exist_ok=True)
        np.savetxt(os.path.join(rest_dir, f"{subject}_Rst.csv"),
                   np.stack(R_list, axis=0), delimiter=",")
...
def load_asu(
    ...
    """Load ASU speech imagery data for one subject.

    Expects CSV files produced by ``_mat_to_csv`` — channel capping to
    ``_ASU_MAX_CHANNELS`` and EOG channel removal (indices ``[0, 9, 32, 63]``)
    are applied during that conversion step, so the CSVs already contain
    clean, flat trial matrices at 256 Hz with bandpass (8–70 Hz) and
    notch (60 Hz) filtering applied.
    ...
    Args:
        subject:   Subject identifier string (e.g. ``'sub_2b'``).
        data_path: Root directory for this dataset split.
        td:        Trial duration in seconds (default 5).
        load_rest: If True, also load and return resting-state data.
    ...
    Returns:
        ``(data_think, targets)`` when *load_rest* is False, or
        ``(data_think, targets, data_rest)`` when True.
        Arrays are flat trial matrices ready for
        ``core_dataset.preprocess_trials``.
    """
    think_path = os.path.join(data_path, "Data",    f"{subject}_Act.csv")
    tgt_path   = os.path.join(data_path, "Targets", f"{subject}_EVE_n.csv")

    data_think = np.loadtxt(think_path, delimiter=",")
    targets    = np.loadtxt(tgt_path,   delimiter=",").astype(int)
    ...
    rest_path = os.path.join(data_path, "Rest", f"{subject}_Rst.csv")
    data_rest = np.loadtxt(rest_path, delimiter=",")
